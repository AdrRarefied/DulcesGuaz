...
const ProductCatalog = () => {
  const handleDownload = () => {
    // Asegúrate de que el archivo se llame exactamente así en /public
    const pdfUrl = "/Catalogo-Dulces-Guaz-2026.pdf"; 
    const link = document.createElement("a");
    link.href = pdfUrl;
    link.setAttribute("download", "Catalogo-Dulces-Guaz-2026.pdf");
      ...
      <div className="container max-w-7xl">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          {products.map((p) => {
            ...
            return (
              <div key={p.name} className="group flex flex-col sm:flex-row bg-white rounded-[2rem] border border-slate-100 overflow-hidden hover:shadow-xl hover:border-primary/20 transition-all duration-500 min-h-[280px]">
                <div className="w-full sm:w-[35%] bg-white p-6 flex items-center justify-center relative border-b sm:border-b-0 sm:border-r border-slate-50">
                  <img src={p.image} alt={p.name} className="w-full h-40 sm:h-full object-contain z-10 group-hover:scale-110 transition-transform duration-700 ease-out" />
                </div>
                    ...
                    <div className="space-y-3">
                      ...
                      <div className="flex flex-wrap gap-2">
                        {listaSabores.map((sabor, i) => (
                          <div key={i} className="flex items-center gap-1.5 bg-slate-50 border border-slate-100 px-2.5 py-1 rounded-lg hover:bg-white hover:border-primary/30 transition-all">
            ...
            <div className="flex justify-center w-full mt-16 mb-8"> 
              <Button 
                asChild
                size="lg"
                className="bg-primary hover:bg-primary/90 text-white rounded-full px-8 py-6 shadow-xl shadow-primary/20 transition-all hover:scale-105 active:scale-95 flex gap-2 group w-fit"
              >
                <a href="/CatalogoDulcesGuaz-2026-.pdf" download="Catalogo-Dulces-Guaz-2026.pdf" className="flex items-center gap-2">
